...
rgabe <- list(
  ...
  tabmetdich <- table(met, dich, dnn = c(gsub("data\\$", "",deparse(substitute(met))), gsub("data\\$", "",deparse(substitute(dich)))))  
  # bei dieser Tabelle werden die Namen erstellt, wobei man fuer dich und met direkt die Spalten aus dem data.frame nehmn kann und trotzdem die Namen
  # richtig in der Tabelle angezeigt werden, ohne das dort data$ vorsteht
)
